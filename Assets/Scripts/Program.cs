...
class Program : MonoBehaviour
{
    ...
    static bool Prebattle(Finite_State_Machine FSM, Save_and_Load<Party> sl, Canvas C)
    {
        ...
        ProgramEvents LoadingScene = C.GetComponentInChildren<ProgramEvents>();
        //teamAturnDisplay TeamATurn = C.GetComponentInChildren<teamAturnDisplay>();
        
        LoadingScene.LoadInformation(FSM, partyA, partyB);
        LoadingScene.gameObject.SetActive(true);
        LoadingScene.Start();
        //TeamATurn.DisableScript();
        //while (LoadingScene.enabled)
        //{
            if (Convert.ToString(FSM.CurrentState) == Convert.ToString(PlayerStates.teamAturn))
            {
                UnityEngine.Debug.Log("butts.");
                teamAturn(FSM, sl, partyA, partyB, C);
            }
        //}
            //Environment.Exit(0);
        return false;
    ...
    static bool teamAturn(Finite_State_Machine FSM, Save_and_Load<Party> sl, Party teamA, Party teamB, Canvas C)
    {
        //UnityEngine.Debug.Log("booty");
        //teamAturnDisplay TeamATurn = C.GetComponentInChildren<teamAturnDisplay>();
        //TeamATurn.EnableScript();
        //TeamATurn.Start();
        //bool FirstUse = false;
        //for (int i = 0; i < 3; i++)
        //{
